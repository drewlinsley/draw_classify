...
class DrawClassifierModel(BaseRecurrent, Initializable, Random):
    def __init__(self, n_iter, reader, 
                    ...
                    decoder_mlp, decoder_rnn, classifier, **kwargs):
        ...
        self.children = [self.reader, self.encoder_mlp, self.encoder_rnn, self.sampler,
                         self.decoder_mlp, self.decoder_rnn, self.classifier]
    ...
    @recurrent(sequences=['u'], contexts=['x'], 
               states=['h_enc', 'c_enc', 'h_dec', 'c_dec'],
               outputs=['c', 'h_enc', 'c_enc', 'i_dec','h_dec', 'c_dec', 'center_y', 'center_x', 'delta'])
    def apply(self, u, h_enc, c_enc, h_dec, c_dec, x):
        r,center_y,center_x,delta = self.reader.apply_simple(x, h_dec)
        ...
        return c, h_enc, c_enc, i_dec, h_dec, c_dec, center_y, center_x, delta
    ...
    @application(inputs=['features'], outputs=['recons', 'h_enc', 'c_enc', 'i_dec', 'h_dec', 'c_dec', 'center_y', 'center_x', 'delta'])
    def reconstruct(self, features):
        ...
        c, h_enc, c_enc, i_dec, h_dec, c_dec, center_y, center_x, delta = \
...
#-----------------------------------------------------------------------------
class LimDrawClassifierModel(BaseRecurrent, Initializable, Random):
    def __init__(self, n_iter, reader, 
                    encoder_mlp, encoder_rnn, sampler,
                    classifier, **kwargs):
        super(LimDrawClassifierModel, self).__init__(**kwargs)   
        self.n_iter = n_iter
    ...
    def get_dim(self, name):
        if name == 'h_enc':
            ...
            super(LimDrawClassifierModel, self).get_dim(name)
    ...
    @application(inputs=['features'], outputs=['recons', 'h_enc', 'c_enc', 'center_y', 'center_x', 'delta'])
    def reconstruct(self, features):
        batch_size = features.shape[0]
        dim_z = self.get_dim('z')
